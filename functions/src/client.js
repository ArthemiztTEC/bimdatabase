...
class LineNotify {
  ...
  sendMessage(message, image_path, sticker_id, package_id) {
    ...
    if (image_path && image_path.match(/^https?:\/\//)) {
      data = { ...data, imageThumbnail: image_path, imageFullsize: image_path }
    } else if (image_path) {
      data = { ...data, imageFile: fs.createReadStream(image_path) }
    }

    if (sticker_id && package_id) {
      data = { ...data, stickerPackageId: package_id, stickerId: sticker_id }
    }

    return this._req(
      `${config.BASE_API_URL}${config.NOTIFY_API_QUERY_PATH}`,
      this.getHeaders(),
      data
    )
  }
  ...
  sendImage(
    image_path = config.DEFAULT_MESSAGE['IMAGE'],
  ...
  sendSticker(
    sticker_id = config.DEFAULT_MESSAGE['STICKER'][0],
